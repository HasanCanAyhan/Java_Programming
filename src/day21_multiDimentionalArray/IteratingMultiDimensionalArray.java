...
public class IteratingMultiDimensionalArray {
    ...
    //  index of 1D Array:    0              1                2
        ...
        for( int[] each1DArray  :  arr2D  ){// we get it arrays inside the arr2D array
        ...
        for (int i = arr2D.length - 1; i >= 0; i--) { // i:  for arrays

            for (int j = arr2D[i].length - 1; j >= 0; j--) { // j : for elements

                System.out.println(arr2D[i][j]); // in the reversed order
